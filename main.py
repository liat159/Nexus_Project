...
def api_create_product(req: ProductCreate, db: Session = Depends(get_db)):
    new_product = create_product(db, req)
    return {"message": "Product created successfully", "id": new_product.id}

# Read - קבלת כל המוצרים למנהל (כולל אלו שנמכרו)
...
def update_product(product_id: str, req: ProductUpdate, db: Session = Depends(get_db)):
    ...
    # חישוב מחדש של מחיר המינימום במידה והעלויות השתנו
